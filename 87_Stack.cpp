...
void storeReverse(string origString, stack<char>& reverseStack)
{
    ...
    {
        reverseStack.push(c); // pushing the element in the stack
        cout<<c<<endl;
    }
}
...
bool isPalindrome(string origString)
{
    ...
    {
        for(char c : origString) // going throught the every character of the origString
        {
            ...
            reverseStack.pop(); // popping the element from the stack
        }
...
void printResult(string origString)
{
    cout<<"Is "<<origString<<" a palidrome?\t"
